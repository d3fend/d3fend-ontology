#!/bin/sh
# Example invocation:
# ./patch-ontology.sh ../d3fend-protege-jun29.ttl ~/Downloads/d3fend-protege-lbs-v5.ttl ~/Downloads/d3fend-protege-66.ttl
...
java -jar robot.jar diff --left $baseline --right $mod --output diff.txt # OWL functional syntax axioms with +/- prefix indicators for add/remove to patch
...
java -jar robot.jar unmerge --input $target             --input unmerge.ofn --output d3fend-unmerged.ttl # Order matters removals only in first --input.
java -jar robot.jar merge   --input d3fend-unmerged.ttl --input merge.ofn   --output d3fend-merged.ttl

# Review d3fend-merged.ttl, replace d3fend-protege.owl with it, and merge that change into feature branch

# Here: rm deletes.ofna inserts.ofna unmerge.ofn merge.ofn d3fend-unmerged.ttl d3fend-merge.ttl diff.txt
