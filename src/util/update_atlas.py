...
def add_atlas_facts(graph):
    ...
    if (d3fend["ATLASThing"], None, None) not in graph:
        graph.add((d3fend["ATLASThing"], RDF.type, owl.Class))
        graph.add((d3fend["ATLASThing"], RDFS.label, Literal("ATLAS Thing")))
        graph.add(
            (d3fend["ATLASThing"], RDFS.subClassOf, d3fend["ExternalThreatModelThing"])
        )

    # 2. Add ATLASTactic
    if (d3fend["ATLASTactic"], None, None) not in graph:
        graph.add((d3fend["ATLASTactic"], RDF.type, owl.Class))
        ...
        graph.add(
            (
                d3fend["ATLASTactic"],
                RDFS.seeAlso,
                URIRef("https://atlas.mitre.org/tactics"),
            )
        )

    # 3. Add ATLASTechnique
    if (d3fend["ATLASTechnique"], None, None) not in graph:
        ...
        graph.add(
            (
                d3fend["ATLASTechnique"],
                RDFS.seeAlso,
                URIRef("https://atlas.mitre.org/techniques"),
            )
        )
        ...
        if atlas_id:
            ...
            if tech.get("x_mitre_is_subtechnique", False):
                ...
                for obj in tech["kill_chain_phases"]:
                    ...
                    fixed_name = name.replace("ATLASAi", "ATLASAI")
...
def update_and_add(graph, data, thesrc):
    ...
    for tactic in thesrc.query(
        ...
        tech_name = str(
            "ATLAS" + string.capwords(tactic["name"]) + " Technique"
        ).replace(" ", "")
        tactic_technique_uri = URIRef(_XMLNS + tech_name.replace("ATLASAi", "ATLASAI"))
