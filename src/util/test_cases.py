...
def test_cases(src, g):
    ...
    # Missing
    _assert(
        g.value(URIRef(_XMLNS + "T1020"), RDFS.label), Literal("Automated Exfiltration")
    )
    _assert(g.value(URIRef(_XMLNS + "T1020"), d3fend["attack-id"]), Literal("T1020"))
    _assert(g.value(URIRef(_XMLNS + "T1020"), RDF.type), owl.Class)

    _assert(
        g.value(URIRef(_XMLNS + "T1055.011"), RDFS.label),
        Literal("Extra Window Memory Injection"),
    )
    _assert(
        g.value(URIRef(_XMLNS + "T1055.011"), d3fend["attack-id"]), Literal("T1055.011")
    )
    _assert(g.value(URIRef(_XMLNS + "T1055.011"), RDF.type), owl.Class)

    # Label Change
    _assert(
        g.value(URIRef(_XMLNS + "T1047"), RDFS.label),
        Literal("Windows Management Instrumentation"),
    )
    _assert(g.value(URIRef(_XMLNS + "T1047"), d3fend["attack-id"]), Literal("T1047"))
    _assert(g.value(URIRef(_XMLNS + "T1047"), RDF.type), owl.Class)

    _assert(
        g.value(URIRef(_XMLNS + "T1053.005"), RDFS.label), Literal("Scheduled Task")
    )
    _assert(
        g.value(URIRef(_XMLNS + "T1053.005"), d3fend["attack-id"]), Literal("T1053.005")
    )
    _assert(g.value(URIRef(_XMLNS + "T1053.005"), RDF.type), owl.Class)

    # Deprecated
    _assert(
        g.value(URIRef(_XMLNS + "T1026"), RDFS.label),
        Literal("Multiband Communication"),
    )
    ...
    _assert(
        g.value(URIRef(_XMLNS + "T1175"), RDFS.label),
        Literal("Component Object Model and Distributed COM"),
    )
    ...
    # Revoked
    _assert(
        g.value(URIRef(_XMLNS + "T1156"), RDFS.label),
        Literal("Malicious Shell Modification"),
    )
    ...
    _assert(g.value(URIRef(_XMLNS + "T1156"), RDFS.seeAlso), d3fend["T1546.004"])
    _assert(
        g.value(URIRef(_XMLNS + "T1156"), RDFS.comment),
        Literal("This technique has been revoked by T1546.004"),
    )

    _assert(
        g.value(URIRef(_XMLNS + "T1066"), RDFS.label),
        Literal("Indicator Removal from Tools"),
    )
    ...
    _assert(g.value(URIRef(_XMLNS + "T1066"), RDFS.seeAlso), d3fend["T1027.005"])
    _assert(
        g.value(URIRef(_XMLNS + "T1066"), RDFS.comment),
        Literal("This technique has been revoked by T1027.005"),
    )
