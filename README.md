...
- make
- java 21, newer versions may work but can emit dependency warnings
- python 3.11
...
|src/dashboard/ |Holds the static dashboard template generated by `make dashboard`.|
...
- `make download-data` downloads the configured external framework source files. `make dashboard` downloads the OCSF, NIST, and CCI sources it needs, combines ontology-backed framework data from atomic SPARQL reports in `src/queries/`, uses the downloaded NIST JSON and CCI XML catalogs as denominator catalogs, writes the assembled JSON report to `reports/dashboard-report.json`, and copies the static dashboard site to `dist/dashboard/`.
- `make update-deps` updates Python dependencies and refreshes `Pipfile.lock`.
